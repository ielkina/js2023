...
//объявление функции
//переменные функций должны отвечать на вопрос что делать что сделать(глагол)
...
add(2, 3);//в скобках передаются параметры переменных функции a, b
add(5, 5);//в скобках передаются параметры переменных функции a, b
...
//Return - полностью прекращает выполнение функции, вне зависимости от вложенности
...
const fn = function (value) {
  ...
  //return в if ... else 
  //когда интерпретатор встречает ретурн он прекращает выполнение функции
  if (value < 50) {
...
//хорошей практикой будет стараться писать функцию так что б она не использовала ничего вне себя, нужно стараться передавать значения в функцию как аргументы
...
const fnC = function () {
  ...
  // console.log(value);//отловка ошибок в коде
}
